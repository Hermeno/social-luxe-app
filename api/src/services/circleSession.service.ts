...
/**
 * Os Círculos que este utilizador pode ver na fila do topo da Feed.
 *
 * Duas passagens, e a distinção entre elas é a razão de a fila existir:
 *
 * **Vivos** — `status: OPEN` dentro da `SESSION_MAX_AGE_MS` que o resto do
 * serviço já usa para decidir se uma sessão ainda é o mesmo momento. Inventar
 * aqui uma segunda definição de "activo" faria a fila mostrar sessões que
 * qualquer outra rota desta API já considera mortas.
 *
 * **Recentes** — sessões já fechadas ou publicadas, que entram só para completar
 * a fila quando não há vivos que cheguem. Chegam com `live: false` e o cliente
 * desliga-lhes o anel cromático: a fila continua a dizer a verdade sobre o que
 * está a acontecer agora, em vez de fingir movimento que não existe.
 *
 * Fronteira de visibilidade, igual nas duas: anfitrião ligado a quem vê — segue
 * ou é seguido — ou uma sessão em que a própria pessoa entrou. É a mesma união
 * que a feed usa. Bloqueio e silêncio cortam nos dois sentidos, e o preenchimento
 * não afrouxa isso: mostrar o Círculo de um estranho por a fila estar curta seria
 * trocar privacidade por decoração.
 *
 * Uma sessão só entra se tiver alguém lá dentro. Um Círculo aberto e vazio é uma
 * intenção, não um momento.
 */
const CIRCLE_BAR_TARGET = 8

export async function activeCircles(userId: string) {
  const fresh = new Date(Date.now() - SESSION_MAX_AGE_MS)

  const [follows, blocksGiven, blocksReceived, mutes] = await Promise.all([
  ...
  for (const follow of follows) {
    connections.add(follow.followerId === userId ? follow.followingId : follow.followerId)
  }
  ...
  const visibleHostIds = [...connections].filter((id) => !hidden.has(id))

  const visibleTo = {
    OR: [
      { hostId: { in: [...visibleHostIds, userId] } },
      { members: { some: { userId, status: 'JOINED' as const } } },
    ],
  }
  const shape = {
    host:    { select: { id: true, name: true, username: true, avatar: true, isVerified: true } },
    members: {
  ...
  const live = await prisma.circleSession.findMany({
    where:   { status: 'OPEN', createdAt: { gte: fresh }, ...visibleTo },
    orderBy: { createdAt: 'desc' },
    take:    CIRCLE_BAR_TARGET,
    include: shape,
  })

  const missing = CIRCLE_BAR_TARGET - live.length
  const recent = missing > 0
    ? await prisma.circleSession.findMany({
        where:   { id: { notIn: live.map((session) => session.id) }, ...visibleTo },
        orderBy: { createdAt: 'desc' },
        take:    missing,
        include: shape,
      })
  ...
  return [
    ...live.map((session) => ({ session, isLive: true })),
    ...recent.map((session) => ({ session, isLive: false })),
  ]
    .filter(({ session }) => !hidden.has(session.hostId) && session.members.length > 0)
    .map(({ session, isLive }) => ({
      sessionId:   session.id,
      host:        session.host,
      // Quatro chegam para a pilha de rostos; o número diz o resto.
      members:     session.members.slice(0, 4).map((member) => member.user),
      memberCount: session.members.length,
      joined:      session.members.some((member) => member.userId === userId),
