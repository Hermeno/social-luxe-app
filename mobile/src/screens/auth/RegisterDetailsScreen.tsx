  ...
  return (
    <KeyboardAvoidingView style={s.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={s.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.form}>
          <TextInput style={s.input} placeholder="Nome de exibição" placeholderTextColor={colors.gray400} value={name} onChangeText={setName} autoCapitalize="words" />
          <View style={s.inputRow}>
            <TextInput style={s.inputFlex} placeholder="Senha (mín. 6 caracteres)" placeholderTextColor={colors.gray400} value={password} onChangeText={setPassword} secureTextEntry={secure} />
          ...
          <TextInput style={s.input} placeholder="Confirmar senha" placeholderTextColor={colors.gray400} value={confirm} onChangeText={setConfirm} secureTextEntry />
          <TouchableOpacity style={[s.btn, (!name || !password || loading) && s.btnOff]} onPress={handleRegister} disabled={!name || !password || loading}>
...
const s = StyleSheet.create({
  ...
  sub:      { fontSize: 14, color: colors.gray400, textAlign: 'center' },
  form:     { gap: spacing.md },
  ...
  input:    { backgroundColor: colors.gray100, borderRadius: radius.md, padding: spacing.md, color: colors.gray800, fontSize: 15 },
  inputFlex:{ flex: 1, padding: spacing.md, color: colors.gray800, fontSize: 15 }, eye: { padding: spacing.sm },
